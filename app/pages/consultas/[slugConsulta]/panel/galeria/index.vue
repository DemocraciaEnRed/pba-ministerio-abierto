...
usePrivatePageSeo('Galería')

const { slug } = useConsultationAdmin()
const toast = useToast()

const basePath = computed(() => `/api/consultations/${slug.value}/gallery`)
...
const { data: images, status, refresh } = await useAsyncData(
  () => `admin-consultation-gallery-${slug.value}`,
...
<template>
  <UPage>
    <UPageHeader
      title="Galería"
      description="Imágenes que se muestran en un carrusel en la página pública de la consulta."
      :links="pageActions"
    />
