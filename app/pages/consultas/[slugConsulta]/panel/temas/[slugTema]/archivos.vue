...
const basePath = computed(() =>
  `/api/consultations/${consultationSlug.value}/topics/${topicSlug.value}/attachments`
)
...
const { data: attachments, status, refresh } = await useAsyncData(
  () => `admin-topic-attachments-${consultationSlug.value}-${topicSlug.value}`,
...
<template>
  <UPage>
    <UPageHeader
      title="Archivos"
      description="Documentos y recursos que la ciudadanía puede descargar en este tema."
      :links="pageActions"
    />
