...
const basePath = computed(() =>
  `/api/consultations/${consultationSlug.value}/topics/${topicSlug.value}/links`
)
...
const { data: links, status, refresh } = await useAsyncData(
  () => `admin-topic-links-${consultationSlug.value}-${topicSlug.value}`,
...
<template>
  <UPage>
    <UPageHeader
      title="Enlaces"
      description="Enlaces relacionados que la ciudadanía ve en este tema."
      :links="pageActions"
    />
