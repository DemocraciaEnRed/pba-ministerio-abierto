...
usePrivatePageSeo('Encuentros - Encuentros Regionales')
...
const { data, status, refresh } = await useAsyncData(
  'encuentros-regionales-consultations',
...
<template>
  <UPage>
    <UPageHeader
      title="Encuentros (Consultas)"
      description="Son aquellas consultas creadas cuya sección son Encuentros Regionales."
    >
      <template #links>
        <UButton
          label="Actualizar"
    ...
    <UPageBody>
      ...
      >
        <p class="text-xs text-muted">
          {{ pagination.total }} {{ pagination.total === 1 ? 'encuentro' : 'encuentros' }}
        </p>
        <UPagination
