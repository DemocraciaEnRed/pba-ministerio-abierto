...
import type { UpdateObservatoryMetricInput } from '#shared/schemas/observatory'
...
const formId = useId()
const formRef = ref<Form<UpdateObservatoryMetricInput>>()
const saving = ref(false)

const state = reactive<UpdateObservatoryMetricInput>({
  label: '',
  value: ''
})
...
async function onSubmit() {
  ...
  try {
    await $fetch(`/api/observatory-metrics/${props.initialValues.id}`, {
      method: 'PATCH',
      body: {
        label: state.label,
        value: state.value
    ...
    <template #body>
      ...
      >
        ...
        <UFormField
          label="Valor"
          name="value"
          help="Texto libre para admitir formatos como «+130»."
          required
        >
          <UInput
            v-model="state.value"
