...
defineOptions({ name: 'EncuentrosRegionalesStepper' })
...
interface AgendaItem {
  ...
  city: string
  date: string
  color: string // clase de fondo del segmento
  year?: number // marca de año al inicio de este segmento
  done?: boolean // el encuentro ya se realizó
}

const items: AgendaItem[] = [
  { name: 'Centro Sur', slug: 'centro-sur', city: 'Tandil', date: '19 de marzo', color: 'bg-region-centro-sur', year: 2026, done: true },
  { name: 'Fluvial', slug: 'fluvial', city: 'San Pedro', date: '14 de mayo', color: 'bg-region-fluvial', done: true },
  { name: 'Este', slug: 'este', city: 'Chascomús', date: '25 de junio', color: 'bg-region-este', done: true },
  { name: 'Noroeste', slug: 'noroeste', city: 'Trenque Lauquen', date: '25 de junio', color: 'bg-region-noroeste', done: true },
  { name: 'Norte', slug: 'norte', city: 'Junín', date: '10 de septiembre', color: 'bg-region-norte' },
  { name: 'Costa Marítima', slug: 'costa-maritima', city: 'Mar del Plata', date: '8 de octubre', color: 'bg-region-costa-maritima' },
  { name: 'Centro Norte', slug: 'centro-norte', city: 'Chivilcoy', date: '5 de noviembre', color: 'bg-region-centro-norte' },
  { name: 'Sudoeste', slug: 'sudoeste', city: 'Bahía Blanca', date: '3 de diciembre', color: 'bg-region-sudoeste', year: 2027 }
]
</script>
    ...
    <div class="hidden lg:grid grid-flow-col auto-cols-fr overflow-x-auto">
      ...
      >
        ...
        <div class="mt-3 text-center leading-tight space-y-2 items-center w-full flex flex-col h-full">
          <p class="text-muted">
            Región
            <span class="block font-bold text-lg text-highlighted">{{ item.name }}</span>
          </p>
        ...
        <div class="flex flex-col gap-2 p-4 w-full leading-tight">
          <div class="flex items-start justify-between gap-2">
            <div class="flex items-center gap-1">
              <p class="text-muted">
                Región
                <span class="font-bold text-lg text-highlighted">{{ item.name }}</span>
              </p>
              <UIcon
                v-if="item.done"
                name="lucide:circle-check"
